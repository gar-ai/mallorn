//! ONNX patch format (.onxp)
//!
//! Binary format for serializing ONNX model patches.
//!
//! ## Format Layout
//!
//! ```text
...
/// Magic bytes for ONNX patch files
pub const ONXP_MAGIC: &[u8; 4] = b"ONXP";

/// Current patch format version
pub const ONXP_VERSION: u32 = 1;
...
pub fn serialize_patch(patch: &Patch) -> Result<Vec<u8>, std::io::Error> {
    ...
    match patch.compression {
        ...
        CompressionMethod::Adaptive { .. } => {
            // Adaptive falls back to Zstd for ONNX
            buf.write_u8(compression_tags::ZSTD)?;
            buf.write_i8(3)?;
        }
...
/// Get file extension for ONNX patches
pub fn extension() -> &'static str {
    "onxp"
}

/// Check if data looks like an ONNX patch
pub fn is_onxp(data: &[u8]) -> bool {
    data.len() >= 4 && &data[0..4] == ONXP_MAGIC
}
...
mod tests {
    ...
    fn make_test_patch() -> Patch {
        Patch {
            ...
            metadata: PatchMetadata {
                ...
                description: Some("Test ONNX patch".into()),
            },
        }
    }
    ...
    fn test_serialize_deserialize_roundtrip() {
        ...
        assert_eq!(&serialized[0..4], ONXP_MAGIC);
    ...
    #[test]
    fn test_is_onxp() {
        assert!(is_onxp(b"ONXP1234"));
        assert!(!is_onxp(b"TFLP1234"));
        assert!(!is_onxp(b"ONX")); // Too short
    }

    #[test]
    fn test_extension() {
        assert_eq!(extension(), "onxp");
    }
    ...
    fn test_all_operation_types() {
        ...
        assert_eq!(deserialized.operations.len(), 4);
    }
}
