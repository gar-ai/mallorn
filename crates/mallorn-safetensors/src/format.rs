//! SafeTensors format definitions and patch serialization
//!
//! SafeTensors model format structure:
//! ┌─────────────────────────────────┐
//! │ Header size (8 bytes, LE u64)   │
//! ├─────────────────────────────────┤
...
/// File extensions for SafeTensors format
pub const SAFETENSORS_EXTENSIONS: &[&str] = &["safetensors"];

/// Patch file extension for SafeTensors
pub const SAFETENSORS_PATCH_EXTENSION: &str = "sftp";

/// Magic bytes for SafeTensors patch files
pub const SFTP_MAGIC: &[u8; 4] = b"SFTP";

/// Current patch format version
pub const SFTP_VERSION: u32 = 1;
...
/// SafeTensors format handler implementing ModelFormat trait
...
impl ModelFormat for SafeTensorsFormat {
    ...
    fn serialize(&self, model: &ParsedModel) -> Result<Vec<u8>, SerializeError> {
        crate::parser::serialize_safetensors(model)
    }
...
/// Get file extension for SafeTensors patches
pub fn extension() -> &'static str {
    SAFETENSORS_PATCH_EXTENSION
}

/// Check if data looks like a SafeTensors patch
pub fn is_sftp(data: &[u8]) -> bool {
    data.len() >= 4 && &data[0..4] == SFTP_MAGIC
}
...
mod tests {
    ...
    fn test_serialize_deserialize_roundtrip() {
        ...
        // Check magic
        assert_eq!(&serialized[0..4], SFTP_MAGIC);
    ...
    #[test]
    fn test_is_sftp() {
        assert!(is_sftp(b"SFTP1234"));
        assert!(!is_sftp(b"TFLP1234"));
        assert!(!is_sftp(b"SFT")); // Too short
    }

    #[test]
    fn test_extension() {
        assert_eq!(extension(), "sftp");
    }
}
