...
//!
//! Binary format for serializing TFLite model patches. Designed for
//! efficient storage and fast parsing on embedded devices.
//!
//! ## Format Layout
//!
//! ```text
...
/// Magic bytes for TFLite patch files
pub const TFLP_MAGIC: &[u8; 4] = b"TFLP";

/// Current patch format version
pub const TFLP_VERSION: u32 = 1;
...
pub fn serialize_patch(patch: &Patch) -> Result<Vec<u8>, std::io::Error> {
    ...
    // Compression method
    let compression_byte = match &patch.compression {
        CompressionMethod::None => compression_tags::NONE,
        CompressionMethod::Zstd { level } => {
            buf.write_u8(compression_tags::ZSTD)?;
            buf.write_i8(*level as i8)?;
            compression_tags::ZSTD // Already written
        }
        CompressionMethod::Lz4 => compression_tags::LZ4,
        CompressionMethod::Neural { .. } => compression_tags::NEURAL,
        ...
        CompressionMethod::ZstdDict { level, dict_id } => {
            ...
            compression_tags::ZSTD_DICT // Already written
        }
    };
...
/// Get file extension for TFLite patches
pub fn extension() -> &'static str {
    "tflp"
}

/// Check if data looks like a TFLite patch
pub fn is_tflp(data: &[u8]) -> bool {
    data.len() >= 4 && &data[0..4] == TFLP_MAGIC
}
...
mod tests {
    ...
    fn test_serialize_deserialize_roundtrip() {
        ...
        // Check magic
        assert_eq!(&serialized[0..4], TFLP_MAGIC);
        ...
        assert_eq!(
            deserialized.metadata.source_version,
            patch.metadata.source_version
        );
        assert_eq!(
            deserialized.metadata.target_version,
            patch.metadata.target_version
        );
    }
    ...
    #[test]
    fn test_is_tflp() {
        assert!(is_tflp(b"TFLP1234"));
        assert!(!is_tflp(b"GGUF1234"));
        assert!(!is_tflp(b"TFL")); // Too short
    }

    #[test]
    fn test_extension() {
        assert_eq!(extension(), "tflp");
    }
    ...
    fn test_all_operation_types() {
        ...
        assert!(matches!(
            ...
            PatchOperation::CopyTensor { name } if name == "copy"
        ));
        assert!(matches!(
            &deserialized.operations[1],
            PatchOperation::ReplaceTensor { name, data, .. } if name == "replace" && data == &[1, 2, 3]
        ));
        assert!(matches!(
            &deserialized.operations[2],
            PatchOperation::DeltaTensor { name, delta_format: DeltaFormat::Xor, .. } if name == "delta_xor"
        ));
        assert!(matches!(
            &deserialized.operations[3],
            PatchOperation::DeltaTensor { name, delta_format: DeltaFormat::BsDiff, .. } if name == "delta_bsdiff"
        ));
        assert!(matches!(
            &deserialized.operations[4],
            PatchOperation::UpdateMetadata { key, value } if key == "key" && value == "value"
        ));
    }
